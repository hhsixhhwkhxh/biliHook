...
public class DownloadPicture extends FunctionsBase {
    ...
    public void run(XC_LoadPackage.LoadPackageParam lpparam) throws Throwable {
        ...
        final Method getPictureUriMethod = Untils.selectMethod(ImageItemParentClass,String.class);

        final Field ImageItemField = Untils.selectField(ImageFragmentClass,ImageItemClass);
        ...
        XposedHelpers.findAndHookMethod(ImageFragmentClass, "onCreateView", LayoutInflater.class, ViewGroup.class, Bundle.class,
            new XC_MethodHook() {
                ...
                protected void afterHookedMethod(final MethodHookParam param) throws Throwable {
                    ...
                    ImageView imageView = view.findViewById(Untils.getViewID("image_view"));
                    imageView.setOnLongClickListener(new OnLongClickListener(){
                            ...
                            public boolean onLongClick(View view) {
                                try {
                                    ...
                                    DownloadManager downloadManager = (DownloadManager) Untils.getMainActivity().getSystemService(Context.DOWNLOAD_SERVICE);
                                    final long downloadId = downloadManager.enqueue(request);
                                    //log("图片开始下载");
                                    Toast.makeText(Untils.getMainActivity(), "开始下载...", Toast.LENGTH_SHORT).show();
                                    ...
                                    BroadcastReceiver receiver = new BroadcastReceiver() {
                                        ...
                                        public void onReceive(Context context, Intent intent) {
                                            ...
                                            if(downloadId==Id){
                                                Toast.makeText(Untils.getMainActivity(), "图片保存于/sdcard/DCIM/BiliBili/", Toast.LENGTH_SHORT).show();
                                            }
                                            // 处理下载完成的逻辑
                                        }
                                    };
                                    Untils.getMainActivity().registerReceiver(receiver, filter);
                                    ...
                                    log("图片保存失败");
                                    log(e);
                                    Toast.makeText(Untils.getMainActivity(), "图片保存失败 请将lsp日志截图报告开发者", Toast.LENGTH_SHORT).show();
                                }
                                return false;
                            }
                        });
